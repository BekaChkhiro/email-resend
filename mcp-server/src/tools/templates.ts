 ...
 */
async function templatesList(args: Record<string, unknown>): Promise<TemplateListResponse> {
  // Validate input
...
registerTool(
  {
    name: "templates_list",
    description:
      "List all templates for a specific campaign. Templates are email body variations used for A/B testing and rotation. Returns templates ordered by sortOrder by default.",
    inputSchema: {
      type: "object",
      properties: {
        campaignId: {
 ...
 */
async function templatesCreate(args: Record<string, unknown>): Promise<TemplateCreateResponse> {
  // Validate input
...
registerTool(
  {
    ...
    description:
      "Create a new email template for a campaign. Templates are email body variations used for A/B testing and round-robin rotation. Campaign must be in 'draft' status. Supports template variables: {{firstName}}, {{lastName}}, {{companyName}}, {{email}}.",
    inputSchema: {
      type: "object",
      properties: {
        campaignId: {
        ...
        body: {
          type: "string",
          description:
            "Email body content (HTML or plain text). Supports variables: {{firstName}}, {{lastName}}, {{companyName}}, {{email}} (required)",
        },
        sortOrder: {
          type: "number",
