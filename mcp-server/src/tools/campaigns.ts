...
import {
  validateCampaignListFilters,
  validateCampaignGet,
  validateCampaignCreate,
  validateCampaignUpdate,
  validateCampaignDelete,
  validateCampaignAddContacts,
...
/**
 * Campaign from Prisma with correct field types
 */
...
/**
 * Email stats aggregation result
 */
interface EmailStatsAggregation {
  campaignId: string;
  total: number;
  pending: number;
  sent: number;
 ...
 */
async function campaignsList(args: Record<string, unknown>): Promise<CampaignListResponse> {
  // Validate input
  ...
  let filteredCampaigns = campaigns as PrismaCampaign[];

  if (filters.minContacts !== undefined) {
    filteredCampaigns = filteredCampaigns.filter(
      (c) => c.selectedContactIds.length >= filters.minContacts!
    );
  }

  if (filters.maxContacts !== undefined) {
    filteredCampaigns = filteredCampaigns.filter(
      (c) => c.selectedContactIds.length <= filters.maxContacts!
    );
  }

  // Get stats if requested
  let statsMap: Map<string, EmailStatsAggregation> = new Map();
  ...
  if (filters.includeStats && filteredCampaigns.length > 0) {
    ...
    for (const campaign of filteredCampaigns) {
      const campaignStats = emailStats.filter(
        (s: { campaignId: string; status: string; _count: number }) => s.campaignId === campaign.id
      );

      const stats: EmailStatsAggregation = {
      ...
      for (const stat of campaignStats) {
        const typedStat = stat as { campaignId: string; status: string; _count: number };
  ...
  const mappedCampaigns: CampaignWithStats[] = filteredCampaigns.map((campaign) => {
    const baseResponse: CampaignWithStats = {
      id: campaign.id,
      name: campaign.name,
...
registerTool(
  {
    name: "campaigns_list",
    description:
      "List campaigns with powerful filtering and search capabilities. Supports filtering by status, email format, AI-enabled campaigns, date ranges, and more. Includes pagination and optional email statistics.",
    inputSchema: {
      type: "object",
      properties: {
        search: {
...
/**
 * Template from Prisma for campaigns_get response
 */
 ...
 */
async function campaignsGet(args: Record<string, unknown>): Promise<CampaignFullResponse | null> {
  // Validate input
  ...
  // Cast to proper type
  const typedCampaign = campaign as PrismaCampaign & {
    templates?: PrismaTemplate[];
  };

  // Build base response
  const response: CampaignFullResponse = {
    id: typedCampaign.id,
  ...
  if (includeTemplates && typedCampaign.templates) {
    response.templates = typedCampaign.templates.map(
      (t: PrismaTemplate): CampaignTemplateResponse => ({
        id: t.id,
  ...
  if (includeStats) {
    ...
    for (const stat of emailStats) {
      const typedStat = stat as { status: string; _count: number };
      const count = typedStat._count;
      stats.total += count;
    ...
    response.stats = {
      ...
      bounceRate:
        stats.total > 0 ? Math.round((stats.bounced / stats.total) * 10000) / 100 : 0,
    };
  }
...
registerTool(
  {
    name: "campaigns_get",
    description:
      "Get a single campaign by ID with full details. Includes campaign templates (sorted by sortOrder) and comprehensive email statistics (sent, delivered, opened, clicked, bounced counts and rates). Returns null if the campaign is not found.",
    inputSchema: {
      type: "object",
      properties: {
        id: {
        ...
        includeStats: {
          type: "boolean",
          description:
            "Include email statistics (total, pending, sent, delivered, opened, clicked, bounced, failed, and calculated rates) - default: true",
        },
      },
      required: ["id"],
    },
 ...
 */
async function campaignsCreate(args: Record<string, unknown>): Promise<CampaignResponse> {
  ...
  if (data.selectedContactIds && data.selectedContactIds.length > 0) {
    ...
    const existingIds = new Set(existingContacts.map((c: { id: string }) => c.id));
  ...
  return {
    ...
    delaySeconds: campaign.delaySeconds,
    status: campaign.status as "draft" | "sending" | "paused" | "completed",
    selectedContactIds: campaign.selectedContactIds,
    aiPrompt: campaign.aiPrompt,
    createdAt: campaign.createdAt,
    sentAt: campaign.sentAt,
...
registerTool(
  {
    ...
    description:
      "Create a new email campaign. The campaign starts in 'draft' status. You can optionally specify schedule settings, AI prompt for personalization, and pre-select contacts.",
    inputSchema: {
      type: "object",
      properties: {
        name: {
        ...
        sendDays: {
          ...
          description:
            "Days of week to send (0=Sunday, 1=Monday, ..., 6=Saturday). Default: [1,2,3,4,5] (Mon-Fri)",
        },
        timezone: {
          type: "string",
          description:
            "IANA timezone for scheduling (e.g., 'America/New_York', 'Europe/London', 'UTC')",
        },
        aiPrompt: {
          type: "string",
          description:
            "AI prompt for generating personalized email content per contact (max 10000 characters)",
        },
        selectedContactIds: {
          type: "array",
          items: {
 ...
 */
async function campaignsUpdate(args: Record<string, unknown>): Promise<CampaignResponse> {
  // Validate input using Zod schema
  ...
  if (updateFields.sendStartHour !== undefined || updateFields.sendEndHour !== undefined) {
    const newStartHour =
      updateFields.sendStartHour !== undefined
        ? updateFields.sendStartHour
        : existingCampaign.sendStartHour;
    const newEndHour =
      updateFields.sendEndHour !== undefined
  ...
  if (updateFields.selectedContactIds && updateFields.selectedContactIds.length > 0) {
    ...
    const existingIds = new Set(existingContacts.map((c: { id: string }) => c.id));
    const missingIds = updateFields.selectedContactIds.filter((cid: string) => !existingIds.has(cid));
  ...
  if (Object.keys(updateData).length === 0) {
    ...
    return {
      ...
      emailFormat: existingCampaign.emailFormat as "html" | "plain_text",
      delaySeconds: existingCampaign.delaySeconds,
      status: existingCampaign.status as "draft" | "sending" | "paused" | "completed",
  ...
  return {
    ...
    emailFormat: updatedCampaign.emailFormat as "html" | "plain_text",
    delaySeconds: updatedCampaign.delaySeconds,
    status: updatedCampaign.status as "draft" | "sending" | "paused" | "completed",
...
registerTool(
  {
    ...
    description:
      "Update an existing campaign. All fields are optional except the campaign ID. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        status: {
          ...
          enum: ["draft", "sending", "paused", "completed"],
          description:
            "New campaign status. Valid transitions: draft→sending, sending→paused/completed, paused→sending/draft",
        },
        sendStartHour: {
          type: "number",
 ...
 */
async function campaignsDelete(args: Record<string, unknown>): Promise<CampaignDeleteResponse> {
  // Validate input
...
registerTool(
  {
    ...
    description:
      "Delete a campaign by ID. This permanently removes the campaign along with all its templates and emails. Cannot delete campaigns that are currently sending - they must be paused or completed first.",
    inputSchema: {
      type: "object",
      properties: {
        id: {
 ...
 */
async function campaignsAddContacts(args: Record<string, unknown>): Promise<CampaignAddContactsResponse> {
  // Validate input using Zod schema
...
registerTool(
  {
    ...
    description:
      "Add contacts to a campaign's selected contacts list. The campaign must be in 'draft' status. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        contactIds: {
          ...
          description:
            "Array of contact UUIDs to add to the campaign (required, max 1000 contacts per call)",
        },
      },
