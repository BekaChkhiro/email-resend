...
import { validateContactListFilters, validateContactCreate, validateContactUpdate, validateContactBulkCreate, } from "../schemas/contacts.js";
// ============================================================================
// contacts_list Tool
// ============================================================================
/**
...
registerTool({
    name: "contacts_list",
    description: "List contacts with powerful filtering and search capabilities. Supports searching by email, name, company, location, industry, and more. Includes pagination for large contact lists.",
    inputSchema: {
        type: "object",
        properties: {
            search: {
 ...
 */
async function contactsGet(args) {
    const id = args.id;
...
registerTool({
    name: "contacts_get",
    description: "Get a single contact by ID. Returns full contact details including the count of campaign emails sent to this contact. Returns null if the contact is not found.",
    inputSchema: {
        type: "object",
        properties: {
            id: {
...
registerTool({
    ...
    description: "Create a new contact. Requires email, firstName, and lastName. All other fields are optional. Returns the created contact or an error if the email already exists.",
    inputSchema: {
        type: "object",
        properties: {
            email: {
...
registerTool({
    ...
    description: "Update an existing contact. Only provided fields will be updated. Returns the updated contact or an error if the contact is not found or if updating email to one that already exists.",
    inputSchema: {
        type: "object",
        properties: {
            id: {
 ...
 */
async function contactsDelete(args) {
    const id = args.id;
    ...
    if (campaignEmailsCount > 0 && !force) {
        throw new ValidationError(`Contact has ${campaignEmailsCount} related campaign email(s). Use force=true to delete anyway.`, {
...
registerTool({
    ...
    description: "Delete a contact by ID. If the contact has related campaign emails, you must set force=true to confirm deletion. This will also delete all campaign email records associated with the contact.",
    inputSchema: {
        type: "object",
        properties: {
            id: {
 ...
 */
async function contactsBulkCreate(args) {
    // Validate input using Zod schema
    ...
    const existingEmails = await withDb(async () => {
        ...
        return new Set(existing.map((c) => c.email.toLowerCase()));
    });
...
registerTool({
    ...
    description: "Bulk create multiple contacts in a single operation. Accepts an array of up to 100 contacts. Use skipDuplicates=true to skip contacts with existing emails instead of failing. Returns counts of created/skipped contacts and any errors.",
    inputSchema: {
        type: "object",
        properties: {
            contacts: {
